...
filter_funcs = { "GS" : vigra.filters.gaussianSmoothing,
                 ...
                 "HGE" :  vigra.filters.hessianOfGaussianEigenvalues,
                 "DG" : difference_of_gaussians,
                 "STE" : structure_tensor_eigenvalues }
...
def main(project_file_path, data_file_path, threads_enabled, thread_num):
    ...
    # Initialize feature times array
    features_times = []
    ...
    for filter_name, sigma, filter_channels in filter_specs:        
        ...
        features_times.append( ["{}({})".format(filter_name,sigma),timer.elapsed_time] )
        
        print "{}({}) took: {} ({} secs/vox el)"\
              .format( filter_name, 
                       sigma, 
                       timer.elapsed_time, 
    ...
    features_times.sort(key=itemgetter(1), reverse=True)
    
    total =  sum([tuple[1] for tuple in features_times])
    
    for tuple in features_times :
        tuple[1] = tuple[1]/total
        
    print sum([tuple[1] for tuple in features_times])
    
    for features_name, time in features_times :
        print features_name + ', ' + str(time)
