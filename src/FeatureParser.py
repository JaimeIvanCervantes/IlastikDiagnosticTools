...
FEATURES_TIMES = { 'STE(10.0)' : 0.0899804176082, \
...
class FeatureParser() :
    ...
    def getFeaturesFromImportanceTable(self, file_name = None, features_importance_table = None ):
        if file_name != None :
        ...
        importances_total = sum([item[1] for item in features_importances.items()])
        
        for feature, importance in features_importances.items():
            features_importances[feature] = importance/importances_total
        
        # Get feature scores taking into account time and importance
        '''
        features_scores = {}    
        ...
        for feature in features_sorted_by_importance :
            features_coordinates.append( (feature[0], FEATURES_COORDINATES[feature[0]]) )    
